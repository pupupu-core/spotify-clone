...
Добавил Docker-образы, настроил Docker Compose и CI. Теперь GitHub автоматически загружает образы фронтенда и бэкенда в GHCR (GitHub Container Registry) — приватное хранилище образов, откуда их можно скачать по токену.

Сейчас для деплоя на VPS мне все еще нужно вручную скачивать эти образы и запускать Docker Compose на сервере, поэтому CD пока не доведено до конца.
...
- `gateway` — слой входа в бэкенд. Здесь находятся все внешние точки входа в систему: REST, GraphQL, WebSocket, static HTML, bots и другие inbound-адаптеры.
